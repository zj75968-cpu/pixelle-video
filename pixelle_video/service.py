...
class PixelleVideoCore:
    ...
    async def _get_or_create_comfykit(self) -> ComfyKit:
        ...
        if (
            self._comfykit is None 
            or self._comfykit_config_hash != current_hash 
            or self._comfykit_loop != current_loop
        ):
            # Close old instance if exists
            if self._comfykit is not None:
                logger.info("🔄 Event loop or ComfyUI configuration changed, recreating ComfyKit instance...")
                try:
    ...
    async def cleanup(self):
        ...
        if self._comfykit:
            ...
            try:
                close_fn = getattr(self._comfykit, "close", None)
                if callable(close_fn):
                    await close_fn()
                    logger.info("✅ ComfyKit session closed")
                else:
                    logger.debug("ComfyKit close() not available, skipping explicit close")
            except Exception as e:
