...
def _render_channel_status_bar() -> None:
    ...
    logger.warning(f"[scraper-debug] username in session={st.session_state.get('username')}, keys={list(st.session_state.keys())}")
    ...
    # 只要安装了 xhs-cli，就显示账号管理与登录 Expander
    if xhs["installed"]:
        with st.expander("🔐 小红书多矩阵账号管理与登录", expanded=False):
            username = st.session_state.get("username") or "default"
            accounts = get_xhs_accounts(username)
            ...
            if accounts:
                ...
                selected_lbl = st.selectbox(
                    ...
                    options, 
                    index=active_idx,
                    key="xhs_active_acc_select"
                )
                
                selected_acc = accounts[options.index(selected_lbl)]["xhs_username"]
                current_active = accounts[active_idx]["xhs_username"]
                if selected_acc != current_active:
                    if switch_xhs_account(username, selected_acc):
                        st.toast(f"已切换激活账号为: {selected_acc}", icon="✅")
                        st.session_state["_ar_status_dirty"] = True
                        st.rerun()
                ...
                for acc in accounts:
                    ...
                    with c_lbl:
                        saved_time_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(acc['saved_at'])) if acc['saved_at'] else '未知'
                        active_tag = " [当前激活]" if acc['active'] else ""
                        st.text(f"• {acc['xhs_username']}{active_tag} (保存于: {saved_time_str})")
                    with c_btn:
                        if st.button("❌ 删除", key=f"del_acc_{acc['xhs_username']}", width="stretch"):
                            delete_xhs_account(username, acc["xhs_username"])
                            st.toast(f"已删除账号: {acc['xhs_username']}", icon="🗑")
                            st.session_state["_ar_status_dirty"] = True
                            st.rerun()
                
                st.markdown("---")
                st.markdown("### ➕ 添加新小红书账号")
            else:
                st.info("💡 当前暂无登录账号，请在下方选择一种方式登录新账号。")
            ...
            with c3:
                if st.button("🔑 自动从浏览器提取 Cookie", key="xhs_do_login", width="stretch"):
                    ...
                    if res["ok"]:
                        xhs_user = register_new_xhs_account(username)
                        st.success(f"✅ 登录并注册成功：{xhs_user or '已激活'}")
                        st.session_state["_ar_status_dirty"] = True
                        st.rerun()
                    else:
            ...
            st.markdown(
                """
                **方式 B：手动输入 Cookie（云端部署/远程访问时唯一可用，非常稳定）**
                
                **🚀 100% 成功获取完整 Cookie 的方法（网络请求提取）**
                1. 电脑浏览器打开并登录 [小红书网页版 (xiaohongshu.com)](https://www.xiaohongshu.com)
                2. 按键盘 **F12** (或页面空白处右键点击「检查」) 打开开发者工具。
                3. 切换到顶部的 **Network (网络)** 选项卡，在下方的过滤器中点击 **Fetch/XHR**。
                4. **刷新网页**，然后在左侧请求列表中找到以 `xiaohongshu.com` 结尾的请求，如 `feed`、`recommend`、`info`、`show` 等。
                5. 点击该请求，在右侧面板的 **Headers (标头) -> Request Headers (请求标头)** 中找到 **`cookie:`** 这一行。
                6. 完整复制后面的整段长文本（右键点击该行选择 `Copy value` 复制），粘贴在下方输入框中并保存。
                
                *⚠️ 注意：请勿使用 Console (控制台) 的 `document.cookie` 提取，因为核心登录参数 `web_session` 带有 `HttpOnly` 安全防护属性，控制台是无法读取到的。*
                """
            )
            
            
            with st.form("xhs_manual_cookie_form", clear_on_submit=False):
                cookie_input = st.text_area(
                    "粘贴小红书 Cookie String",
                    placeholder="例如: a1=1903e...; web_session=04008...;",
                    key="xhs_manual_cookie_input",
                    help="直接粘贴从浏览网络请求标头中复制出来的完整 Cookie 字符串"
                )
                submit_cookie = st.form_submit_button("🔑 保存并激活小红书 Cookie", use_container_width=True)
            ...
            if submit_cookie:
                if not cookie_input.strip():
                    ...
                    cookies = {}
                    # 自动剥离可能带入的 "cookie:" 或 "Cookie:" 前缀
                    cleaned_input = cookie_input.strip()
                    if cleaned_input.lower().startswith("cookie:"):
                        cleaned_input = cleaned_input[7:].strip()
                    ...
                    if "a1" not in cookies:
                    ...
                    elif "web_session" not in cookies:
                        st.error("❌ 缺少核心登录参数 `web_session`！请确保您已在小红书网页端登录，并且在 Network -> Fetch/XHR 中复制了像 `show`、`feed` 或 `recommend` 这种核心请求的 Cookie（不要复制 picasso-static 图片或 collect 统计接口的 Cookie）。")
                    else:
                        with st.spinner("正在保存并激活 Cookie..."):
                            try:
                                ...
                                xhs_user = register_new_xhs_account(username)
                                st.session_state["_ar_status_dirty"] = True
                                st.success(f"✅ Cookie 保存并注册成功：{xhs_user or '已激活'}！")
                                time.sleep(1.0)
                                st.rerun()
            ...
            if st.button("🖥️ 获取/刷新登录二维码", key="xhs_qr_get_btn", width="stretch"):
                with st.spinner("正在获取小红书二维码..."):
                    ...
                    # 等待最多 25 秒以让二维码加载出来（VPS 有头浏览器冷启动通常需要 10-15 秒）
                    for _ in range(50):
            ...
            if session:
                if session.status == "initializing":
                    ...
                    if st.button("🔄 刷新页面状态 (如果已加载完)", key="xhs_qr_refresh_ui_btn", width="stretch"):
                        st.rerun()
                elif session.status == "failed":
                    st.error("❌ 初始化失败。可能受到了防爬虫验证或网络抖动，请点击上方「🖥️ 获取/刷新登录二维码」重试。")
                elif session.status == "expired":
                    st.warning("⚠️ 二维码已过期，请重新点击上方「🖥️ 获取/刷新登录二维码」刷新。")
                            ...
                            if st.button("🔄 检查登录状态", key="xhs_qr_check_btn", type="primary", width="stretch"):
                                if session.status == "logged_in":
                                    try:
                                        ...
                                        st.session_state["_ar_status_dirty"] = True
                                        st.success(f"🎉 扫码登录并注册成功：{xhs_user or '已激活'}！")
                                        time.sleep(1.0)
                                        st.rerun()
                                    except Exception as e:
                                        st.error(f"保存凭证失败: {e}")
