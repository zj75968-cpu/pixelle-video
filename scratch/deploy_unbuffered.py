...
def ssh_connect(max_retries=5):
    for i in range(max_retries):
        try:
            print(f"Connecting to SSH {i+1}/{max_retries}...", flush=True)
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            try:
                ssh.connect(HOST, port=PORT, username=USER, password=PASS, timeout=10)
                print("Connected using standard connection.", flush=True)
                return ssh
            except Exception as e_default:
                print(f"Standard connection failed: {e_default}. Retrying with bound socket...", flush=True)
                s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s.settimeout(15)
                s.bind(("192.168.1.2", 0))
                s.connect((HOST, PORT))
                ssh.connect(HOST, port=PORT, username=USER, password=PASS, sock=s, timeout=15)
                print("Connected using bound socket.", flush=True)
                return ssh
...
def main():
    print("Starting unbuffered deploy process...", flush=True)
    ssh = ssh_connect()
    if not ssh:
        print("Failed to connect to VPS", flush=True)
        sys.exit(1)
    ...
    try:
        ...
        print("\nZipping local project...", flush=True)
        file_count = 0
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for root, dirs, files in os.walk(src_dir):
                ...
                for file in files:
                    ...
                    file_count += 1
        print(f"Zipping complete. Packed {file_count} files into {zip_path}", flush=True)
        
        # 2. Upload zip
        print("\nUploading zip to VPS...", flush=True)
        sftp = ssh.open_sftp()
        ...
        # 3. Extract and Deploy on remote
        print("\nExtracting project on remote...", flush=True)
        ...
        # 4. Fix line endings
        print("\nFixing line endings on remote...", flush=True)
        ...
        print("\nRestarting systemd services on VPS...", flush=True)
        run_cmd(ssh, "systemctl daemon-reload")
        run_cmd(ssh, "systemctl restart pixelle-web pixelle-api")
        
        print("\nWaiting 5s for services to start...", flush=True)
        time.sleep(5)
        
        # 6. Verify
        print("\n=== Verifying Service Status ===", flush=True)
